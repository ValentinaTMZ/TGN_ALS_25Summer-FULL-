...
def plvMatrix(eegData): # input: eeg data per trial
    ...
    for i in range(num_electrodes):
        for j in range(i+1, num_electrodes):
            ...
            plv_matrix[i, j] = plv # np.log(plv + 1e-6)
            plv_matrix[j, i] = plv # np.log(plv + 1e-6)
...
for fold, (train_idx, test_idx) in enumerate(gkf.split(indices, groups=subjects_vec)):
    ...
    # keep the trials of each subject in order
    for v in test_bucket.values():
        v.sort(key=lambda d: int(d.trial_id))
    # one batch per subject
    #test_loader = {sid: [v] for sid, v in test_bucket.items()}
    
    # chunk into mini-batches per subject
    test_loader = {
    ...
    class GraphAttentionEmbedding(torch.nn.Module):
        def __init__(self, in_channels, out_channels, raw_msg_dim, msg_dim, time_enc):
            ...
            self.conv = TransformerConv(in_channels=in_channels,  # node features (likely from memory)
    ...
    @torch.no_grad()
    def test(loader_dict):
        model.eval()
        per_subject = {}
            
        for sid, loader in loader_dict.items():
            model.memory.reset_memory()
            acc_list = []
            #correct = 0
            #total = 0
            ...
            for batch_list in loader:
                ...
                # （TGN 在测试阶段通常也用当前 batch 更新 memory，然后再前向；
                #  如需“纯推理”也可不更新，这里维持与你训练同构的流程）
                model.memory.update_state(batch.src, batch.dst, batch.t, batch.msg)
                out  = model(edge_index, batch.t, batch.msg, n_id, batch_idx)
                pred = out.argmax(dim=1)

                # 该 mini-batch 的 subject 内平均准确率
            ...
            if len(acc_list) > 0:
                per_subject[int(sid)] = {
                    ...
                    "max":  float(np.max(acc_list)),
                    "min":  float(np.min(acc_list)),
                    "n_batches": len(acc_list),
                }
            else:
                per_subject[int(sid)] = {"mean": 0.0, "max": 0.0, "min": 0.0, "n_batches": 0}
        
        # “跨 subject 的均值” 用每个 subject 的 mean 再做一次平均
        mean_over_subjects = float(np.mean([v["mean"] for v in per_subject.values()])) if per_subject else 0.0
        return mean_over_subjects, per_subject

    # ==== run training & evaluation for this fold ====
    best_test = 0.0
    best_per_subject = {}

    for epoch in range(1, E + 1):
        train()
        mean_acc, per_subj = test(test_loader)

        if mean_acc > best_test:
            best_test = mean_acc
            best_per_subject = per_subj

        print(f"[Fold {fold+1}/{K}] Epoch {epoch:02d} | "
              f"Test mean acc={mean_acc:.3f} | Best={best_test:.3f}")

    # record this fold’s best
    cv_best_acc.append(best_test)

    # (optional) show per-subject breakdown at the end of the fold
    print(f"[Fold {fold+1}] best per-subject acc:",
          {int(k): {m: round(v[m], 3) for m in ('mean','max','min')}
           for k, v in best_per_subject.items()})

    # collect per-subject stats across folds (LOSO: each subject appears once)
...
if cv_best_acc:  # only compute if list not empty
    mean_cv = float(np.mean(cv_best_acc))
    std_cv  = float(np.std(cv_best_acc))
    max_cv  = float(np.max(cv_best_acc))
    min_cv  = float(np.min(cv_best_acc))
else:
    mean_cv = std_cv = max_cv = min_cv = 0.0

print(f"Mean={mean_cv:.4f}, Std={std_cv:.4f}, "
      f"Max={max_cv:.4f}, Min={min_cv:.4f}")

# Save fold-level summary
with open('TGN_ALS_cross_sub_batch.json', 'w') as f:
    json.dump({
...
with open('TGN_ALS_cross_sub_batch(per sub).json', 'w') as f:
