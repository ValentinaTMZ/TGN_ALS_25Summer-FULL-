...
def plvMatrix(eegData, fs=256, window_sec=None, overlap_sec=0.1, snap_last=True):
    """
    Only reject a window if its PLV matrix is entirely zeros (i.e., window is completely in gap).
    """
    num_time_steps, num_electrodes = eegData.shape
    
    if window_sec is None:
        raise ValueError("window_sec must be provided (e.g., GLOBAL_MAX_WINDOW_SEC)")
...
def aggregate_temporalData(subject_data, fs=256, GLOBAL_MAX_WINDOW_SEC=None, overlap_sec=0.1):
    """
    ...
    """
    if GLOBAL_MAX_WINDOW_SEC is None:
        raise ValueError("GLOBAL_MAX_WINDOW_SEC must be provided")
    ...
    for label, field in enumerate(['L', 'R']):  # 0 = L, 1 = R
        ...
        # --- concat trials with explicit gap & mask ---
        X, mask, tid, ranges = concat_with_gaps(
            trials=[t[:, :19] for t in trials],   # only use first 19 electrodes
            fs=fs,
            L_max_sec=GLOBAL_MAX_WINDOW_SEC,
        ...
        plv_list, win_starts = plvMatrix(
            eegData=X,
            fs=fs,
            window_sec=GLOBAL_MAX_WINDOW_SEC,
        ...
        for single_plv, t0 in zip(plv_list, win_starts):
            temp_data = temporalData([single_plv], label, t_stamps=[t0]) 
...
GLOBAL_MAX_LEN = float('inf')

# find the max possible window size
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    for label in ['L', 'R']:
        trials = subject_data[label][0]        # shape: [num_trials, time, channels]
        for i in trials:
            trial_len = i.shape[0]         # length in samples
            if trial_len < GLOBAL_MAX_LEN:
                GLOBAL_MAX_LEN = trial_len
                
GLOBAL_MAX_WINDOW_SEC = GLOBAL_MAX_LEN / 256
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    subject_temporal_data = aggregate_temporalData(subject_data=subject_data,
                                                   fs=256, 
                                                   GLOBAL_MAX_WINDOW_SEC=GLOBAL_MAX_WINDOW_SEC, overlap_sec=0)
