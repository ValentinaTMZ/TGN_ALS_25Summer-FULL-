...
with open('TGN_ALSresults(per trial).json', 'r') as file:
...
plt.errorbar(x, gat_means, yerr=[gat_lower, gat_upper], fmt='s',
             color='tab:orange', markersize=20, capsize=18, capthick=4, elinewidth=4, label='trial-length window')

# === Format ===
plt.suptitle('TGN Model Performance Comparison', fontweight='bold', fontsize=38)
plt.title('1-Sec Window vs Trial-Length Window\n(no overlaps, no cross-trial windows)', fontweight='bold', fontsize=33)
...
plt.savefig("1s vs trial-length 11.svg", format='svg')
plt.show()