...
# concat EEG data with GAP + mask + trial_id
def concat_with_gaps(trials: List[np.ndarray], fs: int,
                     window_sec: float, stride_sec: float):
    """
    trials: list of [Ti, C] arrays
    window_sec: fixed window size (sec) used for slicing
    stride_sec: overlap step (sec)
    PADS trial shorter than window_sec with zeros.
    Inserts a gap of length == window_sec after every trial.
    """

    window = int(round(window_sec * fs))
    gap    = window    # ensure gap >= window
    ...
    for i, arr in enumerate(trials):
        ...
        if T < window:
            pad_len = window - T
            padded = np.vstack([arr, np.zeros((pad_len, C), dtype=arr.dtype)])
        else:
            padded = arr
        ...
        M_list.append(np.ones(length, dtype=bool))
        TID_list.append(np.full(length, i, dtype=int))
        ranges.append((cursor, cursor+length, i))
        cursor += length

        # append gap
        gap_arr = np.zeros((gap, C), dtype=arr.dtype)
        X_list.append(gap_arr)
        M_list.append(np.zeros(gap, dtype=bool))
        TID_list.append(np.full(gap, -1, dtype=int))
        cursor += gap
...
def plvMatrix(eegData, fs=256, window_sec=4, overlap_sec=None, snap_last=True):
    """
    Only reject windows that are entirely zero (inside gap).
    """
    
    if overlap_sec is None:
        overlap_sec = 0.05 * window_sec

    num_time_steps, num_electrodes = eegData.shape

    window = int(fs * window_sec)
    hop    = int(fs * (window_sec - overlap_sec))
    if hop <= 0:
        raise ValueError('Overlap must be smaller than window size.')
        
    starts = list(range(0, max(0, num_time_steps - window) + 1, hop))
    
    #snap-to-end：如有尾巴没覆盖，追加一个让最后窗的 end 刚好到 T
    if snap_last and (len(starts) == 0 or starts[-1] + window < num_time_steps):
        last_start = num_time_steps - window
        if len(starts) == 0 or last_start != starts[-1]:
            starts.append(last_start)
    # 现在 starts 覆盖了从 0 到 T 的全程（最后可能与前一窗重叠 > overlap_sec）
    ...
    for start in starts:
        ...
        for i in range(num_electrodes):
            for j in range(i+1, num_electrodes):
                ...
                #plv = np.abs(np.sum(np.exp(1j * phase_diff))) / window
                plv = np.abs(np.mean(np.exp(1j * phase_diff)))
                plv_matrix[i, j] = plv
                plv_matrix[j, i] = plv
                
        # reject only if the window is totally in gap
        if plv_matrix.sum() == 0:
            continue
    ...
    return plv_list, win_starts


# create TemporalData as data object from each single EEG trial, with class label
def temporalData(plv_list, label, t_stamps=None): # import the self-defined timestamp list
    """
    ...
    t_stamps: list/ndarray of ints for each window (e.g., window start sample index).
              If None, uses 0..N-1.
    ...
    if t_stamps is None:
        t_stamps = list(range(len(plv_list)))

    for w, plv in enumerate(plv_list):  # w = window index (used as timestamp)
        t_val = int(t_stamps[w])  # encode actual temporal spacing
...
def aggregate_temporalData(subject_data, 
                           ...
                           overlap_sec=None):
    """
    subject_data: dict with keys 'L' and 'R', each is [1, num_trials] of EEG trials
    Returns: list of TemporalData objects (each window becomes one graph sample)
    """
    if overlap_sec is None:
        overlap_sec = 0.05 * window_sec
    ...
    for label, field in enumerate(['L','R']):
        trials = subject_data[field][0]        # [num_trials, time, channels]

        # each trial -> first 19 electrodes:
        trial_list = [t[:, :19] for t in trials]

        X, mask, tid, ranges = concat_with_gaps(
            trials     = trial_list,
            fs         = fs,
            window_sec = window_sec,
            stride_sec = overlap_sec
        )

        plv_list, win_starts = plvMatrix(
            eegData    = X,
            fs         = fs,
            window_sec = window_sec,
        ...
        if not plv_list:
            print(f"Skipping {field} — no valid windows.")
            continue

        for single_plv, t0 in zip(plv_list, win_starts):
            temp_obj = temporalData([single_plv], label, t_stamps=[t0])
            if (temp_obj.src.numel()==0) or (temp_obj.dst.numel()==0):
                continue
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    subject_temporal_data = aggregate_temporalData(subject_data=subject_data,fs= 256,
                                                   window_sec=4)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(subject_temporal_data)):
        ...
        def train():
            ...
            for batch_list in train_loader:
                model.memory.reset_memory() # reset memory at the beginning of each trial
