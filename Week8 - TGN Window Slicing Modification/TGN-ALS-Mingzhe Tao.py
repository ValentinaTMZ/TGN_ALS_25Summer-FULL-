...
def plvMatrix(eegData, sampling_freq=256, window_sec=6, overlap_sec=1, snap_last=True): 
    # set 1 window size = 3 seconds, overlap 1 second between adjacent windows
    ...
    #num_windows = num_time_steps // window_sec
    
    window = int(sampling_freq * window_sec) # 每个窗口的长度（单位：样本）
    hop = int(sampling_freq * (window_sec - overlap_sec))   # hop: 步长（stride）
    if hop <= 0:
...
def aggregate_temporalData(subject_data, sampling_freq=256, window_sec=6, overlap_sec=1):
    """
    ...
    for label, field in enumerate(['L', 'R']):  # 0 = L, 1 = R
        ...
        plv_list, win_starts = plvMatrix(long_eeg, sampling_freq=sampling_freq, 
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    subject_temporal_data = aggregate_temporalData(subject_data, sampling_freq=256, window_sec=6, overlap_sec=1)
