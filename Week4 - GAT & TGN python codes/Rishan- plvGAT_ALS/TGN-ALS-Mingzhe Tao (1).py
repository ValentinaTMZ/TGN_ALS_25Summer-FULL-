...
def aggregate_temporalData(subject_data, window_size=256):
    ...
    for label, field in enumerate(['L', 'R']):  # 0 for L, 1 for R
        for j in range(subject_data[field].shape[1]):
            eeg_trial = subject_data[field][0, j][:, :19]  # [time, 19]
            plv_list = plvMatrix(eeg_trial, window_size)
            
            if not plv_list:
                print(f"Skipping trial {j} ({field}) — too short for windowing.")
                continue
            
            temp_data = temporalData(plv_list, label)
            
            if temp_data.src.numel() == 0 or temp_data.dst.numel() == 0:
               print(f"Skipping trial {j} ({field}) — empty graph.")
               continue
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    subject_temporal_data = aggregate_temporalData(subject_data, window_size=256) # convert to TemporalData list: event(src, dst, t, msg, label)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(subject_temporal_data)):
        ...
        class CustomTGNMemory(nn.Module): # 1. define a memory module for tgn
          ...
          def __init__(self, num_nodes, raw_msg_dim, memory_dim, time_dim,
              ...
              # no.of nodes to save memory for;
           ...
           self.gru = GRUCell(message_module.out_channels, memory_dim) # update memory using the final aggregated message 

           self.register_buffer('memory', torch.empty(num_nodes, memory_dim)) # create a non-trainable tensor that will store each node’s memory
          ...
          def update_state(self, src, dst, t, raw_msg):
              ...
              with torch.no_grad():
               ...
               agg_msg = self.aggr_module(msg_vec, inv, dim_size=unique_dst.size(0))
               ...
               # Timestamp update
               self.last_update[unique_dst] = t[dst == unique_dst.view(-1,1)].max(dim=1).values
        ...
        class GraphAttentionEmbedding(torch.nn.Module):
            def __init__(self, in_channels, out_channels, msg_dim, time_enc):
                ...
                self.time_enc = time_enc # recall the time encoder module above
                
                self.msg_mlp = nn.Linear(1, msg_dim)
        ...
        class TGNClassifier(nn.Module):
            def __init__(self, num_nodes, raw_msg_dim, memory_dim, time_dim,
                ...
                self.embedding = GraphAttentionEmbedding(in_channels, hidden_channels,
                                                         msg_dim=raw_msg_dim,
        ...
        time_dim = hidden_channels  # <-- ADD THIS LINE
        ...
        model = TGNClassifier(
            ...
            #msg_module=IdentityMessage(out_channels=hidden_channels),
            msg_module=IdentityMessage(raw_msg_dim, hidden_channels, time_dim),
        ...
        for epoch in tqdm(range(1, 50), desc=f"Training Epochs for Subject {subject_number} Fold {fold+1}"):
