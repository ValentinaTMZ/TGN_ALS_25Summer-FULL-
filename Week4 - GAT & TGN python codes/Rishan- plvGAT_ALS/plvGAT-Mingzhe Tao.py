...
for subject_number in subject_numbers: # go through each subject index, process each their .mat files
    mat_fname = pjoin(data_dir, f'OGFS{subject_number}.mat') # pjoin ensures cross-platform compatibility, constructs the full path to each .mat file
...
# %% Phase-GAT
S1 = subject_data['S1'][:,:] # select all rows and all columns from Subject 1


def plvfcn(eegData):
    numElectrodes = eegData.shape[1] 
    numTimeSteps = eegData.shape[0]
    plvMatrix = np.zeros((numElectrodes, numElectrodes)) # initialize PLV matrix, holding the PLV values for every electrode pair
    for electrode1 in range(numElectrodes):
        for electrode2 in range(electrode1 + 1, numElectrodes): # avoid repeating symmetric computations and avoids self-PLV
...
print(f'Mean Highest Test Accuracy Across 10 Folds: {mean_highest_accuracy:.4f}')